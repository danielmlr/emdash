...
In `@emdash-cms/auth`, `createAuthorizationUrl()` and `handleOAuthCallback()` accept `"microsoft"` with a `providers.microsoft` config (`MicrosoftOAuthConfig`), `isValidMicrosoftTenant()` checks a tenant value, and `findOrCreateOAuthUser()` takes a `requireVerifiedEmail` option that refuses self-signup for an unverified address.
