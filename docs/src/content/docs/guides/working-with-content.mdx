...
same time. Scripts, API clients and AI tools connected over MCP that write to a locked entry are
refused; see [Entry edit lock](/reference/rest-api/#entry-edit-lock) in the REST API reference and
[Content lifecycle and bylines](/reference/mcp-server/#content-lifecycle-and-bylines) in the MCP
server reference.
