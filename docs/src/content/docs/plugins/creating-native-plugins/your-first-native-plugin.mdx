...
For a credential that a site administrator enters, declare a `secret` field in the [generated settings form](/plugins/creating-native-plugins/react-admin/#generated-settings-form) instead. EmDash encrypts the value before storing it, and the plugin reads it with `ctx.settings.get()`.
...
The `cloudflare:workers` module resolves only in the Workers runtime, so the hook runs only on Cloudflare.
