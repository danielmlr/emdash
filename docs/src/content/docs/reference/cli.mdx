...
`ENTRY_LOCKED` and names the holder. Wait for them to finish, or pass
`--override-lock`. The same flag is available on `content delete`,
`content publish` and `content unpublish`.
