...
Get a single content item by ID or slug. Returns all field values, metadata, and a `_rev` token. Every tool that changes the item requires that token, so read the item here before writing to it.
...
<Aside>
	Pass the `_rev` token back to `content_update`, `content_publish`, `content_unpublish`, and `content_discard_draft`. Each of them requires it. A write built on a stale token fails with a conflict instead of overwriting a change you have not seen; read the item again and retry with the new token.
</Aside>
...
Update an existing content item. Only include fields you want to change -- unspecified fields are left unchanged. On a published item the change is staged as a draft: the response shows the new values while the live version keeps the old ones until `content_publish`.
