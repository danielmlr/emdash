...
`POST` takes the lock for the caller, or reports who is holding it.
...
lock is taken. The lease expires after a few minutes without a write; a save on
the same entry extends it. Send `{ "takeover": true }` to claim the lock from
whoever holds it. Their next save is then refused, which is how they find out.

`DELETE` releases the lock only when the caller holds it, and reports
`{ "released": false }` otherwise.

While another user holds a live lock, `PUT`, `DELETE`, `/publish`, `/unpublish`
and `/discard-draft` on that entry return `409 ENTRY_LOCKED` with the
holder in `error.details`. Send `"overrideLock": true` in the request body to
write anyway, or `?overrideLock=true` on `DELETE`, which has no body.
